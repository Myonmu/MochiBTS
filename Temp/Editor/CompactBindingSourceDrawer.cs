    ...
    public class CompactBindingSourceDrawer : PropertyDrawer
    {
        ...
        public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
        {
            ...
            if (entry.componentNames is not null) {
                ...
                EditorGUI.BeginChangeCheck();
                property.serializedObject.Update();
                entry.selectedComponentIndex = EditorGUI.Popup(position, entry.selectedComponentIndex, entry.componentNames.ToArray());
                property.serializedObject.ApplyModifiedProperties();
                if (EditorGUI.EndChangeCheck()) {
                    //property.serializedObject.Update();
                    entry.Reflect(property);
            ...
            if (entry.properties is not null) {
                if (entry.properties.Count < 1) {
                    ...
                    EditorGUI.BeginChangeCheck();
                    property.serializedObject.Update();
                    entry.selectedPropertyIndex = EditorGUI.Popup(position, entry.selectedPropertyIndex, entry.properties.ToArray());
                    property.serializedObject.ApplyModifiedProperties();
                    if (EditorGUI.EndChangeCheck()) {
                        property.serializedObject.Update();
                        entry.UpdateSelected(property);
        ...
        protected static CompactBindingSourceEntry Initialize(SerializedProperty prop)
        {
            ...
            */
            ...
            if (entry.propertyObject != null) {
                ...
                entry.resetDelegates = (Action)entry.propertyObject.GetType().GetMethod("ResetDelegate")?.CreateDelegate(typeof(Action), entry.propertyObject);
                ...
                entry.getValueType = (Func<Type>)entry.propertyObject.GetType().GetMethod("GetValueType")?.CreateDelegate(typeof(Func<Type>), entry.propertyObject);
            }
            //initialized = true;
